...
/**
 * A Generic heap class. Unlike Java's priority queue, this heap doesn't just store Comparable
 * objects. Instead, it can store any type of object (represented by type T), along with a priority
 * value. Why do it this way? It will be useful later on in the class...
 */
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
  ...
  /**
   * Returns true if the index corresponds to a valid item. For example, if we have 5 items, then
   * the valid indices are 1, 2, 3, 4, 5. Index 0 is invalid because we leave the 0th entry blank.
   */
  ...
  /** Returns the index of the node with smaller priority. Precondition: not both nodes are null. */
  ...
  private void swim(int index) {
    ...
    while (index > 1 && contents[index].priority() < contents[parIndex].priority()) {
      swap(index, parIndex);
      index = parIndex;
  ...
  private void sink(int index) {
    ...
    while (index < size && (rightChild <= size || leftChild <= size)) {
      int minChild = min(rightChild, leftChild);
      if (contents[index].priority() <= contents[minChild].priority()) {
        break;
      }
      swap(index, minChild);
      index = minChild;
      rightChild = rightIndex(index);
      leftChild = leftIndex(index);
    }
    return;
  }

  /**
   * Inserts an item with the given priority value. This is enqueue, or offer. To implement this
   * method, add it to the end of the ArrayList, then swim it.
   */
  @Override
  ...
  /**
   * Returns the Node with the smallest priority value, but does not remove it from the heap. To
   * implement this, return the item in the 1st position of the ArrayList.
   */
  @Override
  public T peek() {
    if (size > 0) {
  ...
  /**
   * Returns the Node with the smallest priority value, and removes it from the heap. This is
   * dequeue, or poll. To implement this, swap the last item from the heap into the root position,
   * then sink the root. This is equivalent to firing the president of the company, taking the last
   * person on the list on payroll, making them president, and then demoting them repeatedly. Make
   * sure to avoid loitering by nulling out the dead item.
   */
  @Override
  public T removeMin() {
    /* TODO: Your code here! */
  ...
  /**
   * Returns the number of items in the PQ. This is one less than the size of the backing ArrayList
   * because we leave the 0th element empty. This method has been implemented for you.
   */
  @Override
  public int size() {
    return size;
  ...
  /**
   * Change the node in this heap with the given item to have the given priority. You can assume the
   * heap will not have two nodes with the same item. Check item equality with .equals(), not ==.
   * This is a challenging bonus problem, but shouldn't be too hard if you really understand heaps
  ...
  public void changePriority(T item, double priority) {
    ...
    if (!inBounds(index)) { // item not found
      return;
    }
