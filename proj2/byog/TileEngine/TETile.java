 ...
 *
 * <p>All TETile objects must have a character, textcolor, and background color to be used to
 * represent the tile when drawn to the screen. You can also optionally provide a path to an image
 * file of an appropriate size (16x16) to be drawn in place of the unicode representation. If the
 * image path provided cannot be found, draw will fallback to using the provided character and color
 * representation, so you are free to use image tiles on your own computer.
 *
 * <p>The provided TETile is immutable, i.e. none of its instance variables can change. You are
 * welcome to make your TETile class mutable, if you prefer.
 */
public class TETile implements Serializable {
    ...
    /**
     * Constructor without filepath. In this case, filepath will be null, so when drawing, we will
     * not even try to draw an image, and will instead use the provided character and colors.
     *
     * @param character The character displayed on the screen.
     * @param textColor The color of the character itself.
     * @param backgroundColor The color drawn behind the character.
    ...
    /**
     * Draws the tile to the screen at location x, y. If a valid filepath is provided, we draw the
     * image located at that filepath to the screen. Otherwise, we fall back to the character and
     * color representation for the tile.
     *
     * <p>Note that the image provided must be of the right size (16x16). It will not be
    ...
    /**
     * Description of the tile. Useful for displaying mouseover text or testing that two tiles
     * represent the same type of thing.
     *
    ...
    /**
     * Creates a copy of the given tile with a slightly different text color. The new color will
     * have a red value that is within dr of the current red value, and likewise with dg and db.
     *
    ...
    /**
     * Converts the given 2D array to a String. Handy for debugging. Note that since y = 0 is
     * actually the bottom of your world when drawn using the tile rendering engine, this print
     * method has to print in what might seem like backwards order (so that the 0th row gets printed
     * last).
     *
    ...
    @Override
    /** Provides an equals method that is consistent with the way that the autograder works. */
